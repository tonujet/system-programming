...
Car *TaxiCompany::getCar(int carId) const {
    for (Car *car: cars) {
...
Car *TaxiCompany::getRemovedCar(int carId) const {
...
Car *TaxiCompany::addCar(Car *car) {
    ...
    if (isCarAlreadyAdded(car->getId())) return car;
...
Car *TaxiCompany::addRemovedCar(Car *car)
{
...
void TaxiCompany::addRemovedCars(std::vector<Car *> cars)
{
    for(Car *car: cars) {
        addRemovedCar(car);
    }
...
Car *TaxiCompany::increaseMileageInRange(int carId, int min, int max)
{
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> distr(min, max);
    return increaseMileageBy(carId, distr(gen));
}

Car *TaxiCompany::changeOwner(Owner *owner, int carId) {
    Car *car = getCar(carId);
    if (car != nullptr) car->setOwner(owner);
    return car;
}
...
tuple<Car *, int> TaxiCompany::getCarAndPosition(int carId) {
    for (int i = 0; i < cars.size(); i++) {
        Car *car = cars[i];
        if (car->getId() == carId) return {car, i};
    }
...
Car *TaxiCompany::regainCar(int carId) {
    ...
    if (car != nullptr) addCar(car);
    return car;
}

Owner *TaxiCompany::getOwner(int id) const{
    for (Owner *owner: owners) {
